...
function setup() {
    ...
    // Techo
    let ceiling = Matter.Bodies.rectangle(width / 2, 0, width, 200, { isStatic: true });
...
function crearFisicaPantallaInicial() {
    ...
    // Bola física
    bolaBody = Matter.Bodies.circle(0 + width / 2.5, 60, 35, {
        restitution: 0.7,
        friction: 0.05,
        frictionAir: 0.002,
        isStatic: true 
...
function draw() {
    if (gameState === "pantalla_inicial") {
        ...
        noStroke();
        fill(250,200,0);
        text("Aplaudi para elegir una paleta de colores", width / 2, 300);
        ...
        if (bolaEnMovimientoHorizontal) {
            ...
            if (nextX - 35 <  width/2.6 || nextX + 35 > width - width / 2.8) {
                bola.vx *= -1;
            }
    ...
    if ((gestor.derivada > umbralAplauso && cooldown === 0) || reiniciarJuego) {
        for (let i = 0; i < boxes.length; i++) {
    ...
    let nuevoEstado;
    if (amp > 0.08 || estadoSonidoSimulado === "fuerte") {
        nuevoEstado = "fuerte";
    } else if (amp > 0.03 || estadoSonidoSimulado === "medio") {
        nuevoEstado = "medio";
    } else {
    ...
    if ((estadoSonido === "fuerte" || estadoSonido === "medio") && imgIndex !== undefined && millis() - lastBoxTime > BOX_INTERVAL && (crearCaja || amp > 0.03)) {
    ...
    if (flecha.moving) {
        flecha.x += flecha.vx;
        if (flecha.x < width / 3.9 || flecha.x > width - width/3) {
            flecha.vx *= -1;
        }
    }
//cambie el movimiento horizontal de la flecha y la pelota
/*para pasar los datosnomai
   // acá hice cambios
   // ground = Matter.Bodies.rectangle(400, height - 50, 810, 60, { isStatic: true });
    ground = Matter.Bodies.rectangle(width/2, height - 50, width, 200, { isStatic: true });
    Matter.World.add(world, ground);
...
function mousePressed() {
    if (gameState === "pantalla_inicial") return;
    estadoSonidoSimulado = "fuerte";
    crearCaja = true;
}
...
function keyPressed() {
    ...
    if (key === ' ') {
        ...
        estadoSonidoSimulado = "medio";
        crearCaja = true;
    }
}

function mouseReleased() {
    estadoSonidoSimulado = "silencio";
}
