...
let bolaBody = null;
let paletaBodies = [];
let bolaEnMovimientoHorizontal = true; // NUEVO: controla el movimiento horizontal
...
function crearFisicaPantallaInicial() {
    ...
    bolaBody = Matter.Bodies.circle(80, 60, 35, {
        ...
        friction: 0.05,
        frictionAir: 0.002,
        isStatic: true // Empieza estática para controlar el movimiento manualmente
    });
...
function draw() {
    if (gameState === "pantalla_inicial") {
        ...
        if (gestor.derivada > umbralAplauso && cooldown === 0 && bolaEnMovimientoHorizontal) {
    ...
    if (gestor.derivada > umbralAplauso && cooldown === 0) {
        ...
        print("¡Aplauso detectado! Volviendo a pantalla inicial.");
...
function mousePressed() {
    if (gameState === "pantalla_inicial") {
        // Ya no hace nada aquí
        return;
    }
    // ...existing code para el juego...
}